     ...
     {loading
      ...
      : recipes.map((r, index) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          {loading
            ? Array.from({ length: 3 }).map((_, i) => (
              <div key={`sk-recipe-${i}`} className="flex flex-col items-center">
                {/* Skeleton Image */}
                <div className="w-full h-64 rounded-xl bg-white/20 animate-pulse mb-4" />
                {/* Skeleton Text */}
                <div className="w-40 h-7 rounded bg-white/20 animate-pulse mb-2" />
                <div className="w-72 h-20 rounded bg-white/20 animate-pulse" />
              </div>
            ))
            : recipes.map((r, index) => (
              <div
